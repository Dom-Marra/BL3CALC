...
export class PastebinService {
  ...
  getBuild(saveLink: string) {
      ...
      request.open("GET", "http://www.pastebin.com/raw/" + saveLink, true);
  ...
  createBuild(build: Object, character: string) {
    ...
    request.open("POST", "http://www.pastebin.com/api/api_post.php", true);
    ...
    request.onreadystatechange = function() {
      if (request.readyState === 4) {
        ...
        if (response == "Post limit, maximum pastes per 24h reached") {
          ...
          prompt("Here is your build!", "http://localhost:4200/build?character=" + character + "&save=" + response);
        }
      }
    }
  }
